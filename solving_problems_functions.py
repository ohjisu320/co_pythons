...
def problems_main(questions) :      # hint - problem_main으로 변경
    
    #list_problems 에서 질문에만 번호 부여하기 위해 인덱스 홀수 짝수로 나눔
    problems_first = questions[1], questions[3], questions[5], questions[7] # 짝수
    problems_second = questions[0], questions[2], questions[4], questions[6] # 홀수
    
    # 답을 저장할 빈 리스트
    list_results = []

    #각 그룹에서 질문 가져옴
    for question in [0,1,2,3]:       # hint - []-> list_problems 값의 개수
    ...
    def total_responses(score):
        ...
        # 점수 합계
        for i in range(len(input_temp)): # =range(4)
            if list_corrects[i] == input_temp[i]: # input_temp=[4,3,2,3]
                score += score_temp[i]
...
if __name__ == "__main__":
    ...
    # 문제 당 정답
    list_corrects = [2, 1, 1, 2]    # hint - 정답 넣기
